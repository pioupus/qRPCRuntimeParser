...
std::unique_ptr<RPCRuntimeDecodedFunctionCall> RPCRuntimeProtocol::call_and_wait(const RPCRuntimeEncodedFunctionCall &call, std::chrono::steady_clock::duration duration) {
    for (int try_count = 0; try_count <= retries_per_transmission; try_count++) {
        if (try_count != 0) {
            emit console_message(RPCConsoleLevel::debug,
...
bool RPCRuntimeProtocol::load_xml_file(QString search_dir) {
    ...
    auto result = call_and_wait(encoder.encode(0), device_timeout);
